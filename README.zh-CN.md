...
AI 已经很擅长生成漂亮的 HTML——幻灯片、报告、信息图、仪表盘。但当你要在分享前改个标题、换句话时，又得回到代码编辑器里面对一堆标签。

**HTML Studio 把这条链路补上：AI 生成 → 你直接在页面上可视化修改 → 汇报 / 分享。** 不碰代码、不用来回切换。

---

## 它有什么不同

大多数工具止步于"查看" HTML。HTML Studio 让你**像改文档一样改网页**——点击文字直接输入，点击图片直接替换，然后原地保存。三件别人做不到的事：

- **任意 HTML 的所见即所得编辑** —— 通过文本节点遍历自动标记所有可编辑区域（标题、段落、表格单元格、`div`/`span` 文字……），不受页面写法限制。幻灯片还配备翻页导航。
- **母版合并保存** —— 保存时重新从磁盘读取原文件，只合并你改过的部分。脚本、样式和未触碰的内容原样保留。
- **默认即安全** —— 沙箱渲染、5 档安全级别可选，外部链接普通点击不跳转（也不会触发埋点上报）。

如果说 AI 让"生成"内容变得轻而易举，HTML Studio 则让"打磨"它同样简单。
